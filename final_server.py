...
def handle_client(client):
    while True:
        try:
            ...
            if message.startswith("/move"): #first check if they are trying to make a  move in the game
                nickname = nicknames[clients.index(client)]
                move = message.split()[1]
                handle_game_move(nickname, client, move)
            elif message.startswith("/start"): # lets users starts games as lon as two pleople enter "/start"
                if client not in players and len(players) < 2:
                    ...
                    client.send("can't join game at all or more then once".encode('utf-8'))
            else:
                nickname = nicknames[clients.index(client)]
                message = (f"{nickname}: {message}") #adds name to start of message
                broadcast(message) # sends out regular messages
        except:
            remove_client(client)
            break
